...
View::View(Rect p_xScale, Rect p_yScale, Rect p_viewer) : Rect(p_yScale.x(), p_xScale.y(), p_viewer.w() + p_yScale.w(), p_viewer.h() + p_xScale.h(), false) {
	m_xScale = new Scale({ p_xScale.x(), p_xScale.y() }, { p_xScale.w(), p_xScale.h() }, 0, 1000 * PX_SIZE, 25, Orientation::HORIZONTAL);
	m_yScale = new Scale({ p_yScale.x(), p_yScale.y() }, { p_yScale.w(), p_yScale.h() }, 0, 720 * PX_SIZE, 25, Orientation::VERTICAL);
